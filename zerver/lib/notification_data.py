...
class UserMessageNotificationsData:
    ...
    def get_push_notification_trigger(self, acting_user_id: int, idle: bool) -> Optional[str]:
        ...
        # The order here is important. It could happen that, for example both `mention_push_notify`
        # and `stream_push_notify` are True, but we would still want the trigger to be for the mention
        # since, that's more directed towards the user.
    ...
    def get_email_notification_trigger(self, acting_user_id: int, idle: bool) -> Optional[str]:
        ...
        # The order here is important. It could happen that, for example both `mention_email_notify`
        # and `stream_email_notify` are True, but we would still want the trigger to be for the mention
        # since, that's more directed towards the user.
